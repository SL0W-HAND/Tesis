...
La derivación canónica de la radiación de Hawking, como la presentada por Stephen Hawking en su trabajo seminal de 1975, se basa en la falta de un estado de vacío globalmente definido en un espaciotiempo que es dinámico.
...
\begin{figure}[H]
    \begin{small}
        ...
        \end{center}
        \caption{Diagrama de Penrose que ilustra el colapso gravitacional de una estrella (región sombreada) para formar un agujero negro. El espaciotiempo es asintóticamente plano en el pasado nulo ($\mathcal{I}^{-}$) y en el futuro nulo ($\mathcal{I}^{+}$). El horizonte de sucesos ($\mathcal{H}^+$) es la frontera de la región de la que nada puede escapar.}
...
\begin{equation}
    \hat{\phi}=\sum_i\bigl(a_i f_i + a_i^{\dagger} f_i^*\bigr).
\end{equation}
...
Los coeficientes $\alpha_{ji}$ y $\beta_{ji}$ se denominan coeficientes de Bogoliubov. El coeficiente $\alpha_{ji}$ ("alfa") mezcla modos de frecuencia positiva con positiva, mientras que $\beta_{ji}$ ("beta") mezcla modos de positiva "out" con negativa "in".

La pregunta fundamental que planteó Hawking es: dado que el sistema se define en el estado de vacío $\ket{0_{\mathrm{in}}}$ para los observadores en $\calI^-$, ¿cómo perciben este mismo estado los observadores en $\calI^+$? Específicamente, ¿cuál será el número esperado de partículas que detectará un observador en el futuro asintótico ($\calI^+$)?,  el número esperado de partículas ``out'' en el modo $j$ viene dado por
\begin{equation}
...
En este límite, los modos del campo pueden tratarse como rayos de luz (geodésicas nulas) que se propagan hacia atrás en el tiempo desde $\mathcal{I}^{+}$ hasta $\mathcal{I}^{-}$.
...
\end{equation}
El objetivo es descomponer $p_\omega$ en la base "in" (ahora continua)
\begin{equation}
        p_{\omega} = \int_0^{\infty}  \, \left( \alpha_{\omega\omega'} f_{\omega'} + \beta_{\omega\omega'} f_{\omega'}^* \right)d\omega'.
\end{equation}
Para hacer esto se describe un modo critico que viaja hacia atrás en el tiempo desde $\mathcal{I}^{+}$ hasta $\mathcal{I}^{-}$, atravesando la región dinámica del colapso. La relación entre las coordenadas $u$ y $v$ cerca del horizonte de sucesos es crucial para determinar los coeficientes de Bogoliubov.
\begin{figure}[H]
    \begin{small}
        \begin{center}
            \includegraphics[width=0.95\textwidth]{RadiacionDeHawking/figures/rayoluz.png}
        \end{center}
        \caption{}
...
Como se muuestra en la figura \ref{fig:rayo_critico}, se traza un rayo de luz (modo $p_\omega$) hacia atrás en el tiempo desde $\mathcal{I}^{+}$ hasta $\mathcal{I}^{-}$. Los rayos que llegan a $\mathcal{I}^{+}$ en tiempos muy grandes $u$ (cercanos al horizonte) se originan en $\mathcal{I}^{-}$ justo antes de un rayo crítico $v_0$ que forma el horizonte de sucesos. 
...
Mediante las coordenadas de Kruskal-Szekeres, que son regulares en el horizonte
Trazamos un único rayo de luz hacia atrás desde $\mathcal{I}^{+}$ hasta $\mathcal{I}^{-}$. Este rayo está descrito por $U$ en el futuro y $v$ en el pasado. Dado que el rayo es un camino continuo, $U$ debe ser alguna función de $v$, $U=f(v)$. Encontraremos esta función.

La relación entre el tiempo retardado de Schwarzschild $u$ y la coordenada de Kruskal $U$ es:

$$
U=-C_1 e^{-\kappa u}
$$

Ahora, trazamos este mismo rayo (etiquetado por $U$) hacia atrás en el tiempo, a través de la estrella colapsante, hasta llegar a la infinita nulidad pasada $\mathcal{I}^{-}$.
- En $\mathcal{I}^{-}$, nuestra coordenada es el tiempo avanzado $v$ del espacio plano.
Existe un rayo crítico, $v_0$, que es el último rayo "in" que queda atrapado y forma el horizonte de sucesos. Por definición, este rayo crítico $v_0$ debe mapearse al horizonte de sucesos futuro $\mathcal{H}^{+}$. En coordenadas de Kruskal, el horizonte de sucesos futuro $\mathcal{H}^{+}$ es la línea $U=0$. Por lo tanto, por definición: $v_0$ se mapea a $U=0$.
...
\begin{align}
    \alpha_{\omega \omega'} & \sim \int_{-\infty}^{v_0} dv\, (v_0-v)^{\frac{i \omega}{\kappa}} e^{-i \omega' v} = e^{-i\omega'v_0} \int_0^\infty dx\, \left(\frac{x}{\omega'}\right)^{\frac{i\omega}{\kappa}} e^{ix} \frac{1}{\omega'} \nonumber \\
                            & \sim e^{-i\omega'v_0} (\omega')^{-1-i\omega/\kappa} \int_0^\infty dx\, x^{i\omega/\kappa} e^{ix}.
\end{align}
...
El espectro de un cuerpo negro térmico a temperatura $T$ viene dado por la estadística de Bose-Einstein para bosones de energía $E = \hbar\omega$ (reintroduciendo las constantes fundamentales):
\begin{equation}
